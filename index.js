...
io.on('connection', (socket) => {
  ...
  socket.on('shoot', (aimData) => {
    ...
    console.log(`[Shoot] Player ${socket.id} shot at angle ${aimData.angle}`);
  ...
  socket.on('useAbility', () => {
    ...
    // Activate Ability
    if (p.ability === 'arise') {
      console.log(`[Ability] Player ${socket.id} used ARISE`);
      // Summon Minion
      minions.push({
      ...
      console.log(`[Ability] Player ${socket.id} used SCAREDY`);
      // Invisibility
      p.invisible = true;
      ...
      console.log(`[Ability] Player ${socket.id} used SAIYAN`);
      // Super Damage
