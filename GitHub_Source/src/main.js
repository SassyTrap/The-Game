...
let lastSocketEmit = 0; // Throttle timer
...
nameInput.style.color = 'cyan';
nameInput.style.border = '1px solid #444';
nameInput.style.marginRight = '5px';
nameInput.onchange = () => {
    socket.emit('setDetails', { name: nameInput.value, isAnimatronic: false });
};
...
chatInputContainer.appendChild(nameInput);
...
socket.on('playerMoved', (playerInfo) => {
    if (otherPlayers[playerInfo.id]) {
        ...
        // Sync Animation based on MOVEMENT
        if (p.userData.mixer && p.userData.actions) {
            let actionName = 'idle';
            // Threshold: if moved significantly, it's a walk
            if (dist > 0.001) {
                actionName = 'walk';
            }

            // Allow explicit override if provided and not moving? 
            // Or trust movement more? Trust movement.

            const action = p.userData.actions[actionName];
            const current = p.userData.currentAction;

            if (action && current !== action) {
                if (current) current.fadeOut(0.2);
                action.reset().fadeIn(0.2).play();
                p.userData.currentAction = action;
            }
        }
    }
});

socket.on('playerDetailsChanged', (playerInfo) => {
    if (otherPlayers[playerInfo.id]) {
        // Remove old model entirely and re-add to switch Model Type
        scene.remove(otherPlayers[playerInfo.id]);
        delete otherPlayers[playerInfo.id];
        addOtherPlayer(playerInfo); // Re-add with new isAnimatronic flag
    }
});
...
socket.on('playerHealthUpdate', (data) => {
    if (data.id === socket.id) {
        // Local Player Hit
        if (player) {
            player.health = (player.health !== undefined) ? player.health : 100;
            player.health -= data.damage;
            updateLocalHealthUI(player.health);
            console.log("Took Damage. HP: " + player.health);
        }
    } else if (otherPlayers[data.id]) {
        updateHealthBar(otherPlayers[data.id], data.health); // Wait, server sends full health or damage?
        // Server sends { id, damage }. We need to track HP locally for others or server sends HP.
        // Let's assume server just reflects damage for now as we don't have auth logic on server fully.
        // We need to track HP on client for now.
        const p = otherPlayers[data.id];
        p.userData.health = (p.userData.health !== undefined) ? p.userData.health : 100;
        p.userData.health -= data.damage;
        updateHealthBar(p, p.userData.health);

        const hit = p.userData.actions['hit'];
        if (hit) hit.reset().play();
    }
});

socket.on('playerShoot', (data) => {
    // Visual bullet from other player
    if (otherPlayers[data.id]) {
        const origin = otherPlayers[data.id].position.clone();
        origin.y += 1.2; // Lowered from 1.5
        // Calculate direction based on rotation
        const dir = new THREE.Vector3(0, 0, 1).applyEuler(new THREE.Euler(0, data.rotation, 0)); // Rotation is Y axis

        // Spawn standard bullet
        const bulletGeo = new THREE.SphereGeometry(0.1, 8, 8);
        const bulletMat = new THREE.MeshBasicMaterial({ color: 0xffff00 });
        const bullet = new THREE.Mesh(bulletGeo, bulletMat);

        bullet.position.copy(origin);
        bullet.position.add(dir.clone().multiplyScalar(0.5)); // Offset

        // Velocity (Same as local)
        const speed = 0.5;
        bullet.userData.velocity = dir.multiplyScalar(speed);

        scene.add(bullet);
        if (!window.bullets) window.bullets = [];
        window.bullets.push(bullet);

        // Play Shoot Anim on remote
        const p = otherPlayers[data.id];
        if (p.userData.actions && p.userData.actions['shoot']) {
            p.userData.actions['shoot'].reset().play();
        }
    }
});

function createNameTag(name) {
    const tex = createTextTexture(name, '#000000aa');
...
healthContainer.style.backgroundColor = 'red';
healthContainer.style.border = '2px solid black';
healthContainer.style.zIndex = '99999';
...
function updateLocalHealthUI(hp) {
    const pct = Math.max(0, Math.min(100, hp));
    healthFill.style.width = pct + '%';
}

function createHealthBarSprite(hp) {
    ...
    ctx.fillRect(0, 0, (hp / 100) * 128, 16);
    ...
    sprite.position.y = 2.5; // Above Name Tag
...
function updatePlayerNameTag(playerGroup, name) {
    const oldTag = playerGroup.getObjectByName("nameTag");
    if (oldTag) playerGroup.remove(oldTag);

    const tag = createNameTag(name || "Guest");
    tag.name = "nameTag";
    playerGroup.add(tag);

    // Init Health Bar for others
    if (playerGroup.userData.health === undefined) {
        playerGroup.userData.health = 100;
        updateHealthBar(playerGroup, 100);
    }
}

function updateHealthBar(playerGroup, hp) {
    const oldBar = playerGroup.getObjectByName("healthBar");
    if (oldBar) playerGroup.remove(oldBar);
    if (hp <= 0) return;
    const bar = createHealthBarSprite(hp);
    playerGroup.add(bar);
}
...
function addOtherPlayer(playerInfo) {
    ...
    pGroup.position.set(playerInfo.x, playerInfo.y, playerInfo.z);

    let model;

    // Choose Model based on isAnimatronic flag
    if (playerInfo.isAnimatronic && assets.evander) {
        model = SkeletonUtils.clone(assets.evander);
        model.scale.set(1.0, 1.0, 1.0);
        model.position.set(0, -0.5, 0);
        // Setup Evander Anim
        if (assets.evanderAnimations) {
            ...
            assets.evanderAnimations.forEach(clip => {
                ...
                // Map Evander animations
                if (name.includes('walk') || name.includes('run') || name.includes('armature')) actions['walk'] = action;
            });
            // Fallback for Evander
            if (!actions['walk'] && assets.evanderAnimations.length > 0) actions['walk'] = mixer.clipAction(assets.evanderAnimations[0]);

            pGroup.userData.mixer = mixer;
            pGroup.userData.actions = actions;
            pGroup.userData.currentAction = null; // Evander might only have walk
        }
    } else if (assets.guard) {
        model = SkeletonUtils.clone(assets.guard);
        model.scale.set(1.0, 1.0, 1.0);
        model.position.set(0, -0.5, 0);

        // Setup Guard Anim
        if (assets.guardAnimations) {
            const mixer = new THREE.AnimationMixer(model);
            const actions = {};
            ...
            if (idle) {
                idle.play();
                pGroup.userData.currentAction = idle;
            }
        }
    } else {
        // Placeholder
        ...
        pGroup.userData.isPlaceholder = true;
    }

    pGroup.add(model);
    pGroup.scale.set(1.1, 1.1, 1.1); // Match local player root scale

    updatePlayerNameTag(pGroup, playerInfo.name);

    scene.add(pGroup);
    otherPlayers[playerInfo.id] = pGroup;
...
function upgradePlayers() {
    Object.keys(otherPlayers).forEach(id => {
        ...
        if (pGroup.userData.isPlaceholder && assets.guard) {
            // Re-create properly
            scene.remove(pGroup);
            delete otherPlayers[id];
            // We need coords, fortunately pGroup has them
            addOtherPlayer({
                id: id,
                x: pGroup.position.x,
                y: pGroup.position.y,
                z: pGroup.position.z,
                name: "Guest" // Ideally we'd preserve name, but socket update corrects it soon
            });
        }
    });
}
...
async function loadAssets() {
    ...
    const cleanUrl = (url) => url.startsWith('/') ? url.slice(1) : url;
    const load = (url) => new Promise((resolve, reject) => loader.load(cleanUrl(url), resolve, undefined, reject));
    const loadFBX = (url) => new Promise((resolve, reject) => fbxLoader.load(cleanUrl(url), resolve, undefined, reject));

    // Texture Loader
    const texLoader = new THREE.TextureLoader();
    const loadTex = (url) => new Promise((resolve, reject) => texLoader.load(cleanUrl(url), resolve, undefined, reject));
    ...
    try {
        const [table, chair, door, toilet, balloons, computer, pizza, posterPiz, posterBall,
            ...
            doorOpen, doorClosed, lightCeiling, guardModel, evanderModel, kitchenModel] = await Promise.all([
                ...
                load('/Toilet.glb'),
                load('/Balloons.glb'),
                load('/Simple_Computer.glb'),
                load('/Pizza.glb'),
                // Floor tile removed
                loadTex('/Evander_Pizza.png'),
                loadTex('/Evander_Ball.png'),
                load('/Time_Hotel.glb'),
                load('/Curtain.glb'),
                loadFBX('/steps.fbx'),
                load('/Arcade_Machine.glb'),
                load('/Vending_Machine.glb'),
                load('/Counter_Straight.glb'),
                load('/Counter_Corner.glb'),
                load('/Counter_Door.glb'),
                load('/Doorway_Open.glb'),
                load('/Doorway.glb'),
                load('/Light_Ceiling.glb'),
                load('/SWAT.glb'),
                load('/Evander_Animatronic.glb'),
                load('/Kitchen.glb'),
                load('/Pistol.glb')
            ]);
...
function createSecurityGuard() {
    ...
    if (assets.guard) {
        ...
        if (assets.pistol) {
            ...
            if (hand) {
                const gun = assets.pistol.clone();
                // Adjust for SWAT hand
                gun.scale.set(15, 15, 15);
                gun.rotation.set(0, Math.PI, 0);
                gun.position.set(0, 0.1, 0.1);
                hand.add(gun);
            } else {
                console.warn("RightHand bone not found on Guard model");
            }
        }
        ...
        if (assets.guardAnimations && assets.guardAnimations.length > 0) {
            ...
            assets.guardAnimations.forEach(clip => {
                ...
                const name = clip.name.toLowerCase();
                if (name.includes('walk') || name.includes('run')) actions['walk'] = action;
                if (name.includes('idle') && !name.includes('gun')) actions['idle'] = action;

                // Gun Anims (SWAT model usually has them)
                if ((name.includes('pistol') || name.includes('gun')) && name.includes('idle')) actions['aim'] = action; // Idle_Gun_Pointing
                if (name.includes('shoot') || name.includes('fire')) {
                    action.setLoop(THREE.LoopOnce);
                    action.clampWhenFinished = true;
                    actions['shoot'] = action;
                }
                if (name.includes('hit') || name.includes('damage') || name.includes('death')) {
                    action.setLoop(THREE.LoopOnce);
                    actions['hit'] = action;
                }
            });

            // Default to Aiming Idle if available, else Normal Idle
            const startAnim = actions['aim'] || actions['idle'] || mixer.clipAction(assets.guardAnimations[0]);
            if (startAnim) {
                startAnim.play();
                model.userData.currentAction = startAnim;
            }
        }
    ...
    // Flashlight (Only if not holding gun? Or both? Let's assume Gun replaces Flashlight or sits together)
    // GUN ATTACHMENT
    if (assets.pistol) {
        const gun = assets.pistol.clone();
        gun.scale.set(15, 15, 15); // Adjust scale
        gun.rotation.y = Math.PI; // Face forward
        gun.position.set(0, -0.05, 0.15); // In hand
        hR.add(gun);
    }

    // ... Flashlight code kept or removed? keeping purely for backup or removing if interferes.
    // Assuming removing flashlight for gun focus based on prompt.
...
window.addEventListener('keydown', (e) => {
    ...
    // SPACE to Shoot (Guard Only)
    if (e.code === 'Space' && !isAnimatronic && player && player.actions && player.actions['shoot']) {
        // Animation
        const shoot = player.actions['shoot'];
        const current = player.root.children[0].userData.currentAction;
        // Make sure we have Gun attached
        const model = player.root.children[0];
        ...
        // Start at Player + Offset (approx Gun tip)
        // Best: Get world position of Gun if possible, else approx
        const hand = model.getObjectByName('RightHand') || model.getObjectByName('mixamorigRightHand');
        if (hand) {
            const vec = new THREE.Vector3();
            hand.getWorldPosition(vec);
            bullet.position.copy(vec);
            bullet.position.y += 0.05; // Tweaks
        } else {
            bullet.position.copy(player.root.position);
            bullet.position.y += 1.2; // Lowered from 1.5
            const fwd = new THREE.Vector3(0, 0, 1).applyQuaternion(player.root.quaternion);
            bullet.position.add(fwd.multiplyScalar(0.5));
        }
        ...
        dir.applyQuaternion(player.root.quaternion);
        bullet.userData.velocity = dir.multiplyScalar(speed);
        ...
        Object.keys(otherPlayers).forEach(id => {
            if (hitFound) return;
            const pGroup = otherPlayers[id];
            // Intersect with the Group's children (meshes)
            const intersects = raycaster.intersectObjects(pGroup.children, true);
            if (intersects.length > 0 && intersects[0].distance < 50) { // Range limit
                hitFound = true;
                // Deal Damage
                socket.emit('playerDamage', { targetId: id, damage: 20 });
            }
        });
    }
});
...
// Only show if mobile (or for testing)
// if (isMobile()) document.body.appendChild(ToggleBtn); 
document.body.appendChild(ToggleBtn);

let controlsVisible = false;
let joystickContainer, joystickStick, actionBtn;
...
function createJoystick() {
    ...
    joystickContainer.style.zIndex = '999999';
    ...
    actionBtn.innerText = "F";
    actionBtn.style.position = 'fixed';
    actionBtn.style.bottom = '90px';
    ...
    actionBtn.style.border = '2px solid rgba(255, 255, 255, 0.5)';
    ...
    const handleAction = (e) => {
        e.preventDefault();
        keys.f = true;
        if (isAnimatronic) isLightsOn = !isLightsOn;
        setTimeout(() => keys.f = false, 200);
    };
    ...
    const updateStick = (e) => {
        ...
        joystickStick.style.transform = `translate(calc(-50% + ${newX}px), calc(-50% + ${newY}px))`;

        // Update Keys based on Angle (Simple 4-way or 8-way)
        // Angle is in radians. 
        ...
        if (dist > threshold) {
            // Check direction
            // Up/Down
            if (newY < -threshold) keys.w = true;
            if (newY > threshold) keys.s = true;
            // Left/Right
            if (newX < -threshold) keys.a = true;
            if (newX > threshold) keys.d = true;
        }
    };
...
// --- SHOOTING ---
window.addEventListener('keydown', (e) => {
    if (e.code === 'Space' && !player.isStatic) {
        // Shoot
        shootGun();
    }
});
...
if (isMobile()) {
    ...
    btnShoot.innerText = "🔫";
    btnShoot.style.position = 'fixed';
    btnShoot.style.bottom = '180px';
    btnShoot.style.right = '30px';
    btnShoot.style.fontSize = '40px';
    btnShoot.onclick = shootGun;
    document.body.appendChild(btnShoot);
}

function shootGun() {
    if (isAnimatronic) return; // Only guard shoots?

    // Play Animation
    if (player.actions && player.actions['shoot']) {
        player.actions['shoot'].reset().setLoop(THREE.LoopOnce).play();
    }

    // Create Bullet Local
    const origin = player.root.position.clone();
    origin.y += 1.5;
    origin.add(player.root.getWorldDirection(new THREE.Vector3()).multiplyScalar(0.5));

    const dir = new THREE.Vector3(0, 0, 1).applyQuaternion(player.root.quaternion);
    createBullet(origin, dir);

    socket.emit('playerShoot', { rotation: player.root.rotation.y });

    // Check Hit Scanned (Simplified)
    // Raycast forward
    const ray = new THREE.Raycaster(origin, dir, 0, 50);
    const targets = [];
    Object.keys(otherPlayers).forEach(id => targets.push({ id, mesh: otherPlayers[id] }));

    const intersects = ray.intersectObjects(targets.map(t => t.mesh), true); // Recursive
    if (intersects.length > 0) {
        const first = intersects[0];
        // Find which player
        const hitP = targets.find(t => t.mesh === first.object || t.mesh.getObjectById(first.object.id));
        // Actually traverse up to find Group
        let hitObj = first.object;
        while (hitObj.parent && hitObj.parent.type !== 'Scene') {
            const found = targets.find(t => t.mesh === hitObj);
            if (found) {
                // HIT!
                socket.emit('playerHit', { targetId: found.id });
                console.log("Hit player", found.id);
                // Play Hit Sound?
                break;
            }
...
codeInput.addEventListener('change', (e) => {
    if (e.target.value === "Animatronic") {
        ...
        // Notify Server
        socket.emit('setDetails', { name: nameInput.value || "Evander", isAnimatronic: true });
    ...
    try {
        ...
        if (window.bullets) {
            for (let i = window.bullets.length - 1; i >= 0; i--) {
                ...
                // Limit Lifetime/Distance (simple bounds check)
                ...
                Object.keys(otherPlayers).forEach(id => {
                    ...
                    // lenient distance check (Capsule radius approx 0.5, so 1.0 needed center-to-center. 1.5 is safer/easier)
                    // Also check Y height? p.position is at their feet usually.
                    // Bullet height is usually > 1.0.
                    // Let's assume cylinder hit box: distXZ < 1.0 AND 0 < bullet.y < 2.0

                    const distSq = (b.position.x - p.position.x) ** 2 + (b.position.z - p.position.z) ** 2;
                    const dy = b.position.y - p.position.y;

                    if (distSq < 1.5 * 1.5 && dy > 0 && dy < 2.2) {
                        hit = true;
                        // Damage
                        socket.emit('playerDamage', { targetId: id, damage: 20 });
                        ...
                        if (p.children.length > 0) {
                            ...
                            // Simple flash if possible, or just rely on server update
                        }
                    }
                });

                // Collision with World? (Skip for now to prefer player hits)
            }
        }

        renderer.render(scene, camera);
        // Network Update (Throttled to ~30ms to prevent flooding and ensure measurable dist)
        const now = Date.now();
        if (player && socket.connected && (now - lastSocketEmit > 30)) {
            lastSocketEmit = now;
...
loadAssets().then(() => {
    ...
    // Upgrade existing players from placeholder to model
    upgradePlayers();
    ...
    player.health = 100; // Init Health
    updateLocalHealthUI(100);
    scene.add(player.root);
