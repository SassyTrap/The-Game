...
socket.on('playerMoved', (playerInfo) => {
    if (otherPlayers[playerInfo.id]) {
        ...
        // Sync Animation
        if (p.userData.mixer && p.userData.actions && playerInfo.anim) {
            const desiredName = playerInfo.anim.toLowerCase();
            let action = null;

            // Map common names
            if (desiredName.includes('walk') || desiredName.includes('run')) action = p.userData.actions['walk'];
            else if (desiredName.includes('idle')) action = p.userData.actions['idle'];
            else if (p.userData.actions[playerInfo.anim]) action = p.userData.actions[playerInfo.anim];
...
socket.on('playerDetailsChanged', (playerInfo) => {
    if (otherPlayers[playerInfo.id]) {
        updatePlayerNameTag(otherPlayers[playerInfo.id], playerInfo.name);
    }
});
...
function addOtherPlayer(playerInfo) {
    ...
    if (assets.guard) {
        ...
        model.scale.set(1.0, 1.0, 1.0); // Inner scale
        model.position.set(0, -0.5, 0); // Fix floating
