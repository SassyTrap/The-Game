...
socket.on('playerHealthUpdate', (data) => {
    if (otherPlayers[data.id]) {
        updateHealthBar(otherPlayers[data.id], data.health);
...
function createBullet(pos, dir) {
    ...
    const updateBullet = (dt) => {
        age += dt;
        bullet.position.add(dir.clone().multiplyScalar(speed * dt));
        if (age > lifeTime) {
            scene.remove(bullet);
            return false;
        }
...
window.addEventListener('keydown', (e) => {
    ...
    if (e.code === 'Space' && !isAnimatronic && player && player.actions && player.actions['shoot']) {
        ...
        if (current !== shoot) {
            ...
            shoot.reset().fadeIn(0.1).play();
            player.root.children[0].userData.currentAction = shoot;
            // Return to idle after shoot?
            setTimeout(() => {
                if (player.root.children[0].userData.currentAction === shoot) {
                    shoot.fadeOut(0.2);
                    const aim = player.actions['aim'] || player.actions['idle'];
                    if (aim) {
                        aim.reset().fadeIn(0.2).play();
                        player.root.children[0].userData.currentAction = aim;
                    }
                }
            }, 500); // Shoot anim duration approx
        }

        // Logic
        socket.emit('playerShoot'); // Network FX
